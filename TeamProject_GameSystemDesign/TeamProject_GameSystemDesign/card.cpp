...
bool Card::isTileNormal(int& x, int& y) {
    return tileMap[x][y].isNormal;//타일이 노말타입인지 아닌지
}
...
purificationCard::purificationCard(int type, string name, bool isActive, bool isClear): Card(type, name, isActive){
    this.isClear = true;
}
